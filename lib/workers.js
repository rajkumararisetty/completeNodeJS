...
workers.alertUserToStatusChange = (newCheckData) => {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, (error) =>{
    if (!error) {
      ...
      debug("Error: Cloud not send sms alert to user who had a state change");
    }
  });
};
      ...
      checks.forEach((check) => {
        ...
        _data.read("checks", check, (error, checkData) => {
          if (!error && checkData) {
            ...
            debug('Error reading of the check\'s data');
          }
        });
      });
    } else {
      debug("Error cloud not find any checks to process");
    }
  });
};
